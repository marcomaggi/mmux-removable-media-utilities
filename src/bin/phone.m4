...
declare -r script_COPYRIGHT_YEARS='2017'
...
declare script_option_PHONE_MOUNT_POINT=/mnt/phone
...
mbfl_declare_action MAIN MOUNT		NONE mount		'Mount a USB phone.'
mbfl_declare_action MAIN UMOUNT		NONE umount		'Unmount a USB phone.'
mbfl_declare_action MAIN SHOW		NONE show		'Show USB phone mount status.'
...
mbfl_declare_option PHONE_MOUNT_POINT '/mnt/phone' m mount-point witharg 'Select the mount point.'
...
function script_action_MOUNT () {
    ...
    mbfl_message_verbose 'mounting USB phone\n'
    mbfl_program_declare_sudo_user root
    if mbfl_program_exec "$SCRIPT_ARGV0" sudo-mount "$USR_ID" "$GRP_ID" --mount-point="$script_option_PHONE_MOUNT_POINT"
    then
	mbfl_option_verbose && show_mount_point "$script_option_PHONE_MOUNT_POINT"
	exit_success
    else
	mbfl_message_error 'error mounting USB phone'
	exit_failure
    fi
}
function script_action_UMOUNT () {
    mbfl_program_declare_sudo_user root
    mbfl_message_verbose 'unmounting USB phone\n'
    if mbfl_program_exec "$SCRIPT_ARGV0" sudo-umount --mount-point="$script_option_PHONE_MOUNT_POINT"
	# Always try to show the mount point.
    then
	mbfl_option_verbose && show_mount_point "$script_option_PHONE_MOUNT_POINT"
	exit_success
    else
	show_mount_point "$script_option_PHONE_MOUNT_POINT"
	mbfl_message_error 'error unmounting USB phone'
	exit_failure
    fi
}
...
function script_action_SUDO_MOUNT () {
    ...
    if mbfl_program_exec "$MOUNT" "$script_option_PHONE_MOUNT_POINT" -o uid="$USR_ID",gid="$GRP_ID" >&2
    then true
    else
	...
	mbfl_message_verbose 'trying to mount without UID and GID settings\n'
        ...
        # without those.
	if mbfl_program_exec "$MOUNT" "$script_option_PHONE_MOUNT_POINT" >&2
	then true
	else exit_failure
	fi
    fi
...
function script_action_SUDO_UMOUNT () {
    ...
    mbfl_program_exec "$UMOUNT" "$script_option_PHONE_MOUNT_POINT" >&2
}
